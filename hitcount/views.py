...
def _update_hit_count(request, hitcount):
    ...
    try:
        tutorial = TutorialMeta.objects.get(id=hitcount.object_pk)
        tut_author = tutorial.author_set.filter(is_master=True).first().user
    ...
    if user.is_authenticated():
        if not qs.filter(user=user,hitcount=hitcount):
            ...
            RankProcessHelper.create(
                ...
                RankHelper.get_action_by_name('visit'),
            )
